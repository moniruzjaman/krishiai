...
const AIYieldPredictor: React.FC<AIYieldPredictorProps> = ({
	user,
	onAction,
	onBack,
	lang,
	...
	const [aez] = useState("অঞ্চল নির্বাচন করুন");
	const [soilStatus] = useState("মাঝারি উর্বরতা");
	const [practice, setPractice] = useState("সমন্বিত বালাই ব্যবস্থাপনা (IPM)");
	const [water] = useState("পরিমিত সেচ ব্যবস্থা");
		...
		return parts.map((part, i) => {
			if (part.startsWith("[") && part.includes("]")) {
				return (
					<span
						key={i}
						className="block mt-8 mb-3 bg-indigo-50 text-indigo-700 px-4 py-2 rounded-xl font-black text-sm uppercase tracking-widest border border-indigo-100"
					>
						{part.replace(/[\[\]:]/g, "")}
					</span>
				);
			...
			return (
				<span key={i} className="leading-relaxed opacity-90">
					{part}
				</span>
			);
		});
	...
	return (
		<div className="max-w-5xl mx-auto p-4 bg-gray-50 min-h-screen font-sans pb-32 animate-fade-in">
			<ToolGuideHeader
				...
				subtitle="Strategic agronomic forecasting."
				protocol="SAM 3.1"
				source="BARI/BRRI Grounded"
				lang={lang}
				onBack={onBack || (() => {})}
				icon="🔮"
				guideSteps={[]}
					...
					<div className="bg-slate-900 text-white px-8 py-4 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex items-center space-x-4 border border-indigo-500/30 backdrop-blur-md">
						...
						<div className="flex flex-col">
							<span className="text-[10px] font-black uppercase tracking-[0.2em]">
								{yieldLoadingSteps[loadingStep]}
							</span>
							<div className="flex space-x-1 mt-1">
								{[0, 1, 2, 3].map((i) => (
									<div
										key={i}
										className={`h-1 w-4 rounded-full transition-all duration-300 ${i <= loadingStep % 4 ? "bg-indigo-500" : "bg-white/10"}`}
									></div>
								))}
							</div>
						</div>
			...
			{prediction && !isLoading ? (
				<div className="bg-white rounded-[4rem] p-10 md:p-14 border-[12px] border-slate-900 shadow-2xl relative overflow-hidden flex flex-col animate-fade-in mt-10">
					<div className="flex justify-between items-center mb-10 pb-8 border-b-2 border-slate-50 relative z-10">
						...
						<button
							onClick={() => playTTS(prediction)}
							className={`p-5 rounded-full shadow-2xl ${isPlaying ? "bg-rose-500 text-white animate-pulse" : "bg-indigo-600 text-white"}`}
						>
							🔊
						</button>
					</div>
					...
					<div className="mt-16 pt-8 border-t border-slate-50 flex flex-col md:flex-row justify-between items-center gap-4 opacity-60">
						...
						<p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em]">
							Scientific Yield Forecasting Integrity Active
						</p>
					</div>
				</div>
			) : (
				<div className="bg-white rounded-[3rem] p-10 shadow-xl border border-gray-100">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
						...
						<select
							...
							className="p-4 bg-slate-50 border rounded-2xl font-bold"
						>
							<option>IPM পদ্ধতি</option>
							<option>জৈব পদ্ধতি</option>
							<option>সনাতন পদ্ধতি</option>
						</select>
					</div>
					<button
						onClick={handlePredict}
						className="w-full bg-[#0A8A1F] text-white py-6 rounded-3xl font-black text-xl shadow-xl active:scale-95 transition-all"
					>
						পূর্বাভাস জেনারেট করুন
					</button>
				</div>
			)}
		</div>
