...
const ANALYZER_TOUR: TourStep[] = [
	{
		title: "বাংলাদেশি কৃষি রোগ নির্ণায়ন",
		content:
			"এই টুল ব্যবহার করে আপনি কৃষি রোগ, পোকা (Pest), রোগ (Disease) বা পুষ্টি ঘাটতি (Nutrient Deficiency) চিহ্নিত করতে পারবেন।",
		position: "center",
	},
	{
		targetId: "analyzer-media-selector",
		title: "ছবি তুলুন বা গ্যালারি থেকে নিন",
		content:
			"ক্যামেরা বা গ্যালারি থেকে ছবি নির্বাচন করুন, তারপর স্ক্যান বাটনে ক্লিক করুন। আপনি 'লাইভ স্ক্যান' বোতাম ব্যবহার করে সরাসরি স্ক্যান করতে পারেন।",
		position: "top",
	},
];
...
const Analyzer: React.FC<AnalyzerProps> = ({
	...
	const orchestration = getOrchestrationService(); // eslint-disable-line @typescript-eslint/no-unused-vars
	const [sources] = useState<SourceReference[]>([]); // eslint-disable-line @typescript-eslint/no-unused-vars
	const [confidence] = useState<number>(0); // eslint-disable-line @typescript-eslint/no-unused-vars
	const [needsSecondOpinion] = useState<boolean>(false); // eslint-disable-line @typescript-eslint/no-unused-vars
	const [modelUsed] = useState<AIModel>("gemini-2.5-flash"); // eslint-disable-line @typescript-eslint/no-unused-vars
	const [weather, setWeather] = useState<WeatherData | null>(null);
	const [isLiveMode, setIsLiveMode] = useState<boolean>(false);
	const [isLiveStreaming, setIsLiveStreaming] = useState<boolean>(false);
	const [liveTranscription, setLiveTranscription] = useState<string>("");
	const [facingMode] = useState<"environment" | "user">("environment");
	const [isOrchestrationLoading, setIsOrchestrationLoading] =
	...
	const loadingMessages =
		lang === "bn"
			? [
					"এআই অডিট সিস্টেম সক্রিয় করা হচ্ছে...",
					"বহু-মডেল ইন্টেলিজেন্স চালু হচ্ছে...",
					"CABI/BRRI/BARI ডাটাসোর্স লোড হচ্ছে...",
					"ডিপ ভিশন ইঞ্জিন প্রস্তুত হচ্ছে...",
					"রিয়েল-টাইম সিম্পটম বিশ্লেষণ চলছে...",
					"অফিসিয়াল সোর্স অনুয়ায়ী সমাধান খোঁজা হচ্ছে...",
					"বৈজ্ঞানিক ব্যবস্থাপত্র চূড়ান্ত করা হচ্ছে...",
				]
			: [
	...
	const handleAnalyze = async (
		...
		try {
			...
			const prompt =
				lang === "bn" // intentionally unused
					? `Identify problem for ${cropFamily}. NO INTRO. Start immediately with [রোগ নির্ণয়]।`
					: `Identify problem for ${cropFamily}. NO INTRO. Start immediately with [Diagnosis].`;

			const analysisResult = await orchestration.analyzeImage(base64, {
				cropType: cropFamily,
				region: "Dhaka",
				userId: userId,
				lang: lang,
			...
			alert(lang === "bn" ? "বিশ্লেষণ ব্যর্থ হয়েছে।" : "Analysis failed.");
		} finally {
	...
	const handlePrecisionSubmit = async (dynamicData: Record<string, string>) => {
		...
		try {
			...
			const analysis = await orchestration.analyzeImage(base64, {
				cropType: cropFamily,
				region: "Dhaka",
				userId: userId,
				lang: lang,
	...
	return (
		<div className="max-w-4xl mx-auto p-4 pb-32 animate-fade-in font-sans">
					...
					<div className="bg-slate-900/95 text-white p-5 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.4)] flex flex-col space-y-4 border border-emerald-500/30 backdrop-blur-md">
						<div className="flex items-center space-x-4">
							...
							<div className="flex-1 min-w-0">
								<p className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em] mb-1">
									Krishi AI Engine v2.5
								</p>
								<h4 className="text-sm font-bold truncate transition-all duration-500">
						...
						<div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden flex gap-1 px-1 py-0.5">
							{[0, 1, 2, 3, 4, 5].map((i) => (
								<div
									key={i}
									className={`h-full flex-1 rounded-full transition-all duration-500 ${i <= loadingStep ? "bg-emerald-500 shadow-[0_0_8px_#10b981]" : "bg-white/5"}`}
								></div>
							))}
						</div>
					</div>
			...
			<ToolGuideHeader
				title={
					lang === "bn"
						? "অফিসিয়াল বৈজ্ঞানিক অডিট"
						: "Official Scientific Audit"
				}
				subtitle={
					lang === "bn"
						? "Qwen-VL এবং BARI/BRRI ডাটাবেস ব্যবহার করে বৈজ্ঞানিক রোগ নির্ণয়।"
						: "Scientific diagnosis powered by Qwen-VL and BARI/BRRI repositories."
				}
				...
				guideSteps={
					lang === "bn"
						? [
								"ছবি তুলুন বা লাইভ ভিশন ব্যবহার করুন",
								"AI বৈজ্ঞানিক প্রোটোকল তৈরি করে",
								"IPM এবং সঠিক MoA ডোজ শিখুন",
							]
						: [
								"Capture photo or use Live Vision.",
								"AI generates scientific protocol.",
								"Learn IPM and correct MoA dosage.",
							]
				}
			/>
			...
			<div className="bg-white rounded-[3rem] p-6 md:p-10 shadow-xl border border-slate-100 mb-8 print:hidden">
				<div className="space-y-6">
					<select
						...
						className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl py-4 px-6 font-black text-lg text-slate-700 outline-none focus:border-emerald-500 appearance-none"
					>
							...
							{isLiveStreaming ? (
								<div className="w-full h-full rounded-[2.5rem] overflow-hidden border-4 border-emerald-500 shadow-2xl relative bg-black">
									...
									<div className="absolute top-4 left-4 z-30">
										<span className="bg-rose-600 text-white text-[8px] font-black px-2 py-1 rounded-full animate-pulse uppercase tracking-widest">
											Live Scan Active
										</span>
									</div>
									<div className="absolute bottom-6 left-0 right-0 flex justify-center px-6 z-30">
										<button
											...
											className="w-full bg-red-600 text-white py-4 rounded-2xl font-black text-xs uppercase shadow-xl"
										>
											বন্ধ করুন
										</button>
									</div>
								...
								<div className="w-full h-full rounded-[2.5rem] overflow-hidden border-4 border-emerald-50 shadow-2xl relative bg-black">
									...
									<div className="absolute bottom-6 left-0 right-0 flex justify-center space-x-4 px-6 z-30">
										<button
											...
											className="flex-[2] bg-emerald-600 text-white py-4 rounded-2xl font-black text-xs uppercase shadow-xl"
										>
											অডিট করুন
										</button>
										<button
											onClick={stopLiveMode}
											className="p-4 bg-red-600 text-white rounded-2xl"
										>
											✕
										</button>
									</div>
								...
								<div className="grid grid-cols-2 gap-3 h-full">
									<button
										...
										className="col-span-2 bg-slate-900 rounded-[2.5rem] border-4 border-slate-800 flex flex-col items-center justify-center space-y-3 hover:bg-black transition-all"
									>
										<div className="text-4xl">🛰️</div>
										<p className="text-[10px] font-black text-white uppercase tracking-widest">
											{lang === "bn" ? "লাইভ ভিশন AI" : "Live Vision AI"}
										</p>
									</button>
									<button
										onClick={startLiveMode}
										className="bg-emerald-50 rounded-[2.5rem] border-4 border-dashed border-emerald-200 flex flex-col items-center justify-center space-y-2 hover:border-emerald-500 transition-all"
									>
									...
									<button
										...
										className="bg-rose-50 rounded-[2.5rem] border-4 border-dashed border-rose-200 flex flex-col items-center justify-center space-y-2 hover:border-rose-500 transition-all"
									>
								...
								<div className="flex items-center justify-between mt-4 gap-2">
									...
									<button
										...
										disabled={
											...
											isLiveStreaming ||
											(!selectedMedia && !isLiveMode)
										}
										className="flex-1 bg-emerald-600 text-white py-5 rounded-[1.8rem] font-black text-[10px] uppercase shadow-xl disabled:opacity-50 active:scale-95 transition-all"
									>
										বৈজ্ঞানিক অডিট করুন
									</button>
								</div>
			...
			{precisionFields && !isLoading && !result && (
				<DynamicPrecisionForm
					...
					isLoading={isLoading}
					toolProtocol="SCIENTIFIC-SCAN-V5"
				/>
			)}
			...
			{result && !isLoading && (
				<div className="space-y-8 animate-fade-in-up">
					<div
						ref={reportRef}
						className="bg-white rounded-none border-[12px] border-slate-900 p-8 md:p-14 shadow-2xl relative overflow-hidden flex flex-col print:border-[5px]"
					>
						...
						<div className="flex items-center justify-between mb-6 px-4 py-2 bg-slate-50 border border-slate-200 rounded-2xl">
							<div className="flex items-center space-x-2">
								<div
									className={`w-3 h-3 rounded-full ${result.confidence >= 0.8 ? "bg-emerald-500" : result.confidence >= 0.6 ? "bg-amber-500" : "bg-rose-500"} animate-pulse`}
								></div>
								<span className="text-[10px] font-black uppercase text-slate-500 tracking-wider">
									Confidence: {(result.confidence * 100).toFixed(0)}% | Model:{" "}
									{result.model}
								</span>
							</div>
						...
						<div className="flex flex-col md:flex-row justify-between items-start border-b-4 border-slate-900 pb-10 mb-10 gap-8">
							...
							<div className="flex space-x-2 print:hidden">
								<button
									...
									className="p-4 rounded-2xl bg-white text-emerald-600 border border-emerald-100 shadow-sm transition-all active:scale-90"
								>
									Share
								</button>
								<button
									...
									disabled={isSaving}
									className="p-4 rounded-2xl bg-slate-900 text-white shadow-xl transition-all active:scale-90"
								>
									Save
								</button>
								<button
									onClick={() =>
										playSpeech(result.fullText || result.diagnosis)
									}
									className={`p-4 rounded-full shadow-lg ${isSpeaking ? "bg-rose-500 animate-pulse" : "bg-emerald-600 text-white"}`}
								>
									🔊
								</button>
							</div>
						...
						<div className="grid grid-cols-1 lg:grid-cols-12 gap-12 relative z-10">
							<div className="lg:col-span-12 prose prose-slate max-w-none text-slate-800 font-medium leading-[1.8] whitespace-pre-wrap text-xl">
								<div className="mb-8">
									<h4 className="text-xs font-black uppercase text-emerald-600 mb-4 tracking-widest border-l-4 border-emerald-600 pl-4">
										[Symptoms]
									</h4>
								...
								<div className="mb-8 bg-emerald-50/50 p-8 rounded-[2rem] border border-emerald-100">
									<h4 className="text-xs font-black uppercase text-emerald-600 mb-4 tracking-widest">
										[Scientific Advisory]
									</h4>
								...
								{result.management.chemical.length > 0 && (
									<div className="mb-8 p-8 bg-slate-50 rounded-[2rem] border border-slate-200">
										<h4 className="text-xs font-black uppercase text-slate-500 mb-4 tracking-widest">
											[Chemical Control - DAE Recommended]
										</h4>
											...
											{result.management.chemical.map((chem, i) => (
												...
												>
													...
													<p className="text-sm opacity-70">
														Dosage: {chem.concentration} | Interval:{" "}
														{chem.interval}
													</p>
												</div>
											))}
								...
								{result.sources.length > 0 && (
									<div className="mt-12 pt-8 border-t-2 border-slate-100">
										<h4 className="text-xs font-black uppercase text-slate-400 mb-6 tracking-widest">
										...
										<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
											{result.sources.map((source, i) => (
												<a
													...
													className="flex flex-col p-4 bg-slate-50 rounded-2xl hover:bg-emerald-50 transition-colors border border-slate-200 group"
												>
													<div className="flex justify-between items-start mb-2">
														<span className="text-[10px] font-black text-emerald-600 uppercase transform transition-transform group-hover:scale-105">
													...
													<p className="text-[11px] text-slate-500 line-clamp-2 mt-1 leading-relaxed italic">
														"{source.excerpt}"
													</p>
												</a>
											))}
						...
						<div className="mt-16 pt-8 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 opacity-50">
							...
							<div className="text-center md:text-right">
								<p className="text-[9px] font-bold text-slate-300 uppercase tracking-[0.2em]">
									Scientific Audit Active • Grounded Research
								</p>
							</div>
						</div>
					</div>
