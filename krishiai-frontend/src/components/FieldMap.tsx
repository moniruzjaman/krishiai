          ...
          <div id="main-field-map" className="bg-white rounded-[3rem] p-4 shadow-2xl border border-slate-100 overflow-hidden aspect-video relative group transition-all duration-500">
            ...
            <div className="absolute top-8 right-8 flex flex-col space-y-2">
              <button
                ...
                className={`p-4 rounded-2xl shadow-xl border transition-all active:scale-90 flex items-center justify-center ${isTracking
              ...
              {isLoading && !sellers ? (
                ...
                sellers.groundingChunks.map((chunk, idx) => chunk.maps ? (
                  <div
                    ...
                    className={`p-5 rounded-[2.2rem] border transition-all cursor-pointer group ${selectedPlaceQuery === chunk.maps.title
...
const MapActionBtn = ({ icon, label, onClick, active }: any) => (
  <button
    onClick={onClick}
    className={`px-6 py-4 rounded-2xl shadow-sm transition-all flex items-center space-x-3 active:scale-95 group border-2 ${active
