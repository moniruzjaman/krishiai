...
const CROP_SYMPTOM_DB: Record<string, Record<string, string[]>> = {
  'rice': {
    'yellowing_leaves': ['Pest: Brown Plant Hopper', 'Disease: Bacterial Blight', 'Deficiency: Nitrogen'],
    'stunted_growth': ['Deficiency: Zinc', 'Pest: Root Knot Nematode', 'Disease: Sheath Blight'],
    'brown_spots': ['Disease: Leaf Blast', 'Pest: Rice Stem Borer', 'Deficiency: Potassium']
  },
  'wheat': {
  ...
  'general': {
    'yellowing': ['Deficiency: Nitrogen', 'Pest: Aphids', 'Disease: Viral infection'],
    'wilting': ['Pest: Root damage', 'Disease: Wilt disease', 'Water stress'],
    'spots': ['Disease: Fungal infection', 'Pest: Leaf miner', 'Deficiency: Magnesium']
  }
};
...
export const getRuleBasedAnalysis = (cropFamily: string, symptoms: string[]): AnalysisResult => {
  ...
  const category = matches.length > 0
    ? matches[0].includes('Pest') ? 'Pest' :
      matches[0].includes('Disease') ? 'Disease' : 'Deficiency'
    : 'Other';
  ...
  return {
    ...
    source: 'Rule-Based Expert System (Bangladesh Agriculture)',
    audioBase64: null,
    groundingChunks: []
  };
};
