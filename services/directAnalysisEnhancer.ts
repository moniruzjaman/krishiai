...
export class DirectAnalysisEnhancer {
   ...
   */
  enhanceAnalysis(baseResult: AnalysisResult, context: DirectAnalysisContext = {}): AnalysisResult {
    const { cropFamily = 'General', userQuery = '', lang = 'bn', confidenceThreshold = 65 } = context;
   ...
   */
  private addOfficialSources(result: AnalysisResult, cropFamily: string, lang: string): AnalysisResult {
    ...
    const source = sourceMap[cropKey] || 'Bangladesh Agricultural Research Council (BARC)';
    const sourceUrl = cropKey === 'rice' ? 'https://dae.gov.bd/krishi-janala-2024' :
                     cropKey === 'wheat' ? 'https://bari.gov.bd/wheat-disease-guide' :
                     cropKey === 'potato' ? 'https://bari.gov.bd/potato-disease-guide' :
  ...
  private ruleBasedEnhancement(
    ...
    // Extract symptoms from user query or base result
    const symptoms = this.extractSymptoms(userQuery || baseResult.diagnosis);

    // Use our enhanced rule-based database
    const enhancedResult = this.getEnhancedRuleBasedDiagnosis(cropFamily, symptoms, lang);
    ...
    return {
      ...
      management: `${enhancedResult.management}\n\n${baseResult.management || ''}`.trim(),
  ...
  private extractSymptoms(text: string): string[] {
    const symptomKeywords = [
      'yellow', 'yellowing', 'pale', 'light', 'faded',
      'brown', 'black', 'spot', 'spots', 'lesion', 'lesions',
      'white', 'gray', 'mold', 'fungus',
      'stunted', 'dwarf', 'small', 'short',
      'wilting', 'drooping', 'collapse',
      'curl', 'curling', 'twist', 'deform',
      'hole', 'holes', 'chewed', 'eaten',
      'rot', 'decay', 'soft', 'mushy'
    ];
  ...
  /**
   * Get enhanced rule-based diagnosis (simplified version)
   */
  private getEnhancedRuleBasedDiagnosis(cropFamily: string, symptoms: string[], lang: string): AnalysisResult {
    // Simplified version - in production, this would use our comprehensive CSV data
    ...
    if (crop.includes('rice') && symptoms.some(s => s.includes('yellow'))) {
      ...
      confidence = 75;
    } else if (crop.includes('rice') && symptoms.some(s => s.includes('stunt'))) {
      ...
      confidence = 70;
    } else if (symptoms.some(s => s.includes('spot')) || symptoms.some(s => s.includes('lesion'))) {
    ...
    return {
      ...
      groundingChunks: [{
        web: {
          ...
          uri: source.includes('dae.gov.bd') ? 'https://dae.gov.bd' :
               source.includes('bari.gov.bd') ? 'https://bari.gov.bd' :
               source.includes('barc.gov.bd') ? 'https://barc.gov.bd' : ''
        }
      }]
    };
  }
