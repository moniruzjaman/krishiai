...
const aiResult: AnalysisResult = {
  ...
  category: 'Mixed',
  management: 'Apply general fertilizer and pesticide',
...
const enhancedResult = directAnalysisEnhancer.enhanceAnalysis(aiResult, userInput);
...
console.log(`   Contains Bengali characters: ${/[বাংলা]/.test(enhancedResult.management)}`);

// Verify authentic sources
console.log('\n5. Source Verification:');
console.log(`   Source URL pattern: ${enhancedResult.source.includes('dae.gov.bd') || enhancedResult.source.includes('bari.gov.bd') || enhancedResult.source.includes('brri.gov.bd') || enhancedResult.source.includes('barc.gov.bd') ? '✓ Authentic Bangladesh source' : '✗ Generic source'}`);
...
const fallbackResult = directAnalysisEnhancer.enhanceAnalysis(lowConfidenceResult, userInput);
