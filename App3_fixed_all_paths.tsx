...
import { SettingsProvider } from './context/SettingsContext';  // Changed from '@/context/SettingsContext'
import ModelSettings from './components/ModelSettings';  // Changed from '../components/ModelSettings'
import { View, User, SavedReport, Language, UserRole } from './types';  // Fixed import path
...
import { syncUserProfile, saveReportToSupabase, getOrCreateUser, fetchUserReports } from './services/supabase';
...
const AppContent: React.FC = () => {
  ...
  const [user, setUser] = useState<User>({
    ...
    role: 'farmer_entrepreneur' as UserRole,
    progress: {
      rank: 'নবিশ কৃষক',
      level: 1,
      xp: 120,
  ...
  // ... (speech synthesis verification code) ...
  ...
  }, [user.displayName, user.role, user.farmLocation, user.progress, user.preferredCategories]);
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case View.PROFILE:
        return <UserProfile user={user} onUpdateUser={(updates: Partial<User>) => setUser((prev: User) => ({ ...prev, ...updates }))} onSaveReport={handleSaveReport} onToggleSpeech={() => setSpeechEnabled(!speechEnabled)} speechEnabled={speechEnabled} onBack={() => handleNavigate(View.HOME)} lang={lang} />;
      case View.ABOUT:
      ...
      <div className={`min-h-screen transition-colors duration-500 ${user.settings?.theme === 'dark' ? 'dark bg-slate-950' : 'bg-slate-50'}`}>
        ...
        <header className="fixed top-0 left-0 right-0 h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800 z-[100] px-4 flex items-center justify-between shadow-sm">
          <div className="flex items-center space-x-3">
            <button onClick={() => setIsDrawerOpen(true)} aria-label="Open menu" className="p-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl hover:bg-emerald-50 transition-all active:scale-90 shadow-sm border border-slate-100 dark:border-slate-700">
